...
#####################################################################
#
#   Projective equivalence classes maker (Schreier Tree)
#
#####################################################################

# Stores the value of the parent object, and the generator index that
# got to this node.
struct TransversalEntry
    parent_hash::UInt64
    gen_idx::Int

    function TransversalEntry(parent_hash::UInt64, gen_idx::Int)
        return new(parent_hash, gen_idx)
    end 
end

# Outer constructor for shorter name
trans_node(parent_hash::UInt64, gen_idx::Int) = TransversalEntry(parent_hash, gen_idx)


# Finding a representation of the matrix that takes the starting point to 
# this new point gotten from node, i.e. M such that point*M = new_point.
function orbit_traversal(transversal::Dict{UInt64, TransversalEntry}, G_gens, 
    root_hash::UInt64, target_hash::UInt64)
 
    res = identity_matrix(G_gens[1])
    
    curr_hash = target_hash

    while curr_hash != root_hash
        node = transversal[curr_hash]
        mul!(res, G_gens[node.gen_idx], res) 
...
# Creates a hash of a vector (of type FqMatrix) as an integer (UInt64). 
...
function orbit_stabilizer!(buffers, G_gens, point, action_dict)

    empty!(buffers.orbit_queue)
    empty!(buffers.schreier_tree)
...
    buffers.schreier_tree[init_key] = trans_node(init_key, 0) 

    temp_coords = buffers.temp_coords
    M_parent = parent(point)

    while !isempty(buffers.orbit_queue)
        vec, trans_vec = popfirst!(buffers.orbit_queue)
        vec_coords = vec.v
        packed_v = pack_vector(vec_coords, q, buffer_elem)

        for (idx, g_original) in enumerate(G_gens)
            M_linear = action_dict[g_original]

            mul!(temp_coords, vec_coords, M_linear)
            packed_key = pack_vector(temp_coords, q, buffer_elem)

            if !haskey(buffers.schreier_tree, packed_key)
                next_vec = M_parent(deepcopy(temp_coords))
                trans_next = trans_vec * g_original

                push!(buffers.orbit_queue, (next_vec, trans_next))
                buffers.schreier_tree[packed_key] = trans_node(packed_v, idx)
            else
                trans_next = orbit_traversal(buffers.schreier_tree, G_gens, init_key, packed_key)
...
                next_vec = M_parent(deepcopy(temp_coords))
                push!(buffers.orbit_queue, next_vec)
            end
        end
    end
    return
...
            buffers = (
                schreier_tree = Dict{UInt64, TransversalEntry}(),
                orbit_queue = Vector{Tuple{elem_type(W),FqMatrix}}(),
                stab_gens = Set{FqMatrix}(),
                temp_coords = zero_matrix(F, 1, dim_W),
                identity = identity_mat,
            )
            sizehint!(buffers.schreier_tree, 256)
...
                        stab_vec_lift = orbit_stabilizer!(buffers, orbits_stabilizers_dict[vec], cand_vec, global_action_cache)
                        partial_orbit_stab_dict[cand_vec] = stab_vec_lift
                        union!(hash_space, keys(buffers.schreier_tree))
                    end
                end
            end
...
# A final function that returns a set of vector representatives, in V, for 
...
    buffers = (
        schreier_tree = Dict{UInt64, TransversalEntry}(),
        orbit_queue = Vector{Tuple{elem_type(W_2),FqMatrix}}(),
        stab_gens = Set{FqMatrix}(),
...
            stab_gens = orbit_stabilizer!(buffers, GL_gens, vec, action_dict)
...
    
    delete!(poly_reps, zero(V))

    return (poly, poly_reps)
end
